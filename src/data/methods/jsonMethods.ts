...
import { IDataStorageMethods, IDataSaveObject, TDataArray, IRawUserProfile } from "../../interfaces/IDataStructures"
import { IClassNoun, IClassPreposition, IClassAdverb, IClassAdjective, IWord } from "../../interfaces/IWordsPhrasesGrammar"
import { isValidWordClassAdjective, isValidWordClassAdverb, isValidWordClassNoun, isValidWordClassPreposition } from '../../util/dataValidations';
...
export class JSONMethods implements IDataStorageMethods {
  ...
  private jsonReader = async (fileName: string): Promise<(TDataArray | IRawUserProfile)> => fs
    .promises
    .readFile(path.join(process.cwd(), 'json', `data.${fileName}.json`))
    .then(data => JSON.parse(data.toString()))

  private jsonWriter = async (fileName: string, data: (TDataArray | IRawUserProfile)): Promise<void> => fs
    .promises
    .writeFile(path.join(process.cwd(), 'json', `data.${fileName}.json`), JSON.stringify(data, null, 2))
}
